...
export const useAccountSummaryDetailed = (chainId: number = 11155111): DetailedAccountSummary => {
  ...
  const summary = useMemo(() => {
    if (!supplies || !borrows || !allMarkets || isUserDataPending || isMarketDataPending) {
      return {
    ...
    supplies.forEach((supply, index) => {
      if (supply.balance > BigInt(0) && marketData) {
        ...
        if (marketIndex >= 0) {
          ...
          if (supplyRateData?.status === 'success' && exchangeRateData?.status === 'success') {
            ...
            const underlyingBalance = Number(formatUnits(supply.balance, 8)) * exchangeRate;
  ...
  }, [supplies, borrows, allMarkets, marketData, accountLiquidityData, isUserDataPending, isMarketDataPending, liquidityError]);
