...
export const useAccountSummaryWithOracle = (chainId: number = 11155111): AccountSummaryWithOracle => {
  ...
  const summary = useMemo(() => {
    if (!supplies || !borrows || !allMarkets || !marketData || !oracleData || 
        isUserDataPending || isMarketDataPending || isOraclePending) {
      return {
    ...
    supplies.forEach((supply) => {
      if (supply.balance > BigInt(0)) {
        const marketIndex = (allMarkets as Address[]).findIndex(m => m === supply.cToken);
        if (marketIndex >= 0) {
          ...
          if (supplyRateData?.status === 'success' && 
            ...
            const exchangeRate = Number(exchangeRateData.result as bigint) / 1e18;
            ...
            const cTokenBalance = Number(formatUnits(supply.balance, cTokenDecimals));
    ...
    borrows.forEach((borrow) => {
      if (borrow.currentBalance > BigInt(0)) {
        const marketIndex = (allMarkets as Address[]).findIndex(m => m === borrow.cToken);
        if (marketIndex >= 0) {
    ...
    const avgSupplyAPY = totalSupplyWeight > 0 ? totalSupplyAPYWeighted / totalSupplyWeight : 0;
    const avgBorrowAPY = totalBorrowWeight > 0 ? totalBorrowAPYWeighted / totalBorrowWeight : 0;
  ...
  }, [supplies, borrows, allMarkets, marketData, oracleData, accountLiquidityData, 
      isUserDataPending, isMarketDataPending, isOraclePending, liquidityError]);

  return {
    ...summary,
    isLoading: isUserDataPending || isLiquidityPending || isMarketDataPending || isOraclePending,
  };
}; 