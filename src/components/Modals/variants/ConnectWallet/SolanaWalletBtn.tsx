...
export const SolanaWalletBtn: React.FC<WalletBtnPros> = ({ name, icon, index }) => {
        ...
        if (solWallet) {
          if (connected && name == wallet?.adapter.name) {
            ...
            if (solWallet.readyState == "NotDetected") {
              ...
              solWallet.readyState == "Installed" || solWallet.readyState == "Loadable"
            ) {
    ...
    >
      ...
      >
        ...
        <span
          className={cx(
            styles.status,
            (walletState == "Installed" || walletState == "Loadable") && styles.highlight,
          )}
        >
          {
            (name == wallet?.adapter.name) ? (connected ? "Disconnect" : (connecting ? "Connecting" : ""))
              : (walletState == "Installed" || walletState == "Loadable") ? "Connect"
                : (walletState == "NotDetected" ? "Not detected" : "Unsupported")
          }
        </span>
      </Text>
    </Button>
