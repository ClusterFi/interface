...
import { Button, CurrencyIcon, Heading, Icon, Loader, Text } from "@/components/shared";
...
export const Status: React.FC<StatusProps> = ({ state }) => {
	switch (state) {
			...
			return (
				<div className={styles.container}>
					<div className={styles.inner}>
						{state === "loading" ? <Loader width={60} height={60} /> : <Icon glyph={"Succeed"} width={60} height={60} />}
		...
		case "error":
			return (
				<div className={styles.container}>
					<Icon className={styles.errorIcon} glyph={"Error"} width={60} height={60} />
					<Heading element="h3" className={styles.errorTitle}>
						Something went wrong
					</Heading>
					<Button className={styles.errorButton} size={"large"} variant={"stroke"}>
						Back
					</Button>
				</div>
			);
