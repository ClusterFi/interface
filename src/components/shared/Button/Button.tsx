...
export function Button<E extends React.ElementType = typeof defaultElement>({ children, color, variant, size, as, className, disabled, isActive, ...rest }: ButtonProps<E>) {
	const TagName = as || defaultElement;

	return (
		<>
			<TagName className={cx(styles.button, styles[size], styles[variant], color && styles["color-" + color], disabled && styles.disabled, isActive && styles.isActive, className)} disabled={disabled} {...rest}>
