...
import { Heading, Container, Overview, Section, Text, Table, CurrencyIcon, Button, Icon } from "@/components";
			...
			<Container className={styles.container}>
				...
				{mounted && (
					<div className={styles.slider}>
						<Button disabled={isBeginning} className={styles.prev} onClick={onSlidePrev} size="custom" variant="gradient-light">
							<Icon glyph={"Arrow"} width={24} height={24} />
						</Button>
						<Button disabled={isEnd} className={styles.next} onClick={onSlideNext} size="custom" variant="gradient-light">
							<Icon glyph={"Arrow"} width={24} height={24} />
						</Button>
						<Swiper
								...
								<SwiperSlide className={styles.slide} key={index}>
									<div className={styles.token}>
										<div className={styles.tokenIcon}>
											<CurrencyIcon width={40} height={40} currency={item.token} />
											<CurrencyIcon width={18} height={18} currency={item.network} />
										</div>
										<Heading element="h4" className={styles.tokenName}>
											{item.name}
										</Heading>
										<Text size={14} theme={400} className={styles.tokenShortname}>
											{item.shortName}
										</Text>
									</div>
				...
				<div className={styles.grid}>
					<Market isLoading={controls["markets-loading"]} currency={"Ethereum"} name={"Ethereum"} />
					<Market isLoading={controls["markets-loading"]} currency={"Solana"} name={"Solana"} />
					<Market isLoading={controls["markets-loading"]} currency={"Hyperliquid"} name={"Hyperliquid"} />
				</div>
			</Container>
		</section>
	);
